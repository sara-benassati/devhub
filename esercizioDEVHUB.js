...
app.put('/addUser', async (req, res) => {
  try {
    ...
    const existing_User = await db.collection('users').findOne({ username: username});
    if(existingUser) return res.status(404).json({rc: 1, msg: `User ${username} already exists`})
    // effettuo lo stesso controllo anche per il campo email
  const existing_email = await db.collection('emails').findOne({ email: email});
    if(existing_email) return res.status(404).json({rc: 1, msg: `email ${email} already exists`})
